...
-- ======================================================
-- Crafting Room Monitor v4 (Map-Focused Edition, GEO-aware)
...
-- - NEW: Can render GEO scanner output from:
...
--      using a top-down slice at Roomva's Y-level.
-- ======================================================
...
local fuelyID        = 10              -- Fixy turtle computer ID
    ...
    if fs.exists(geoEdgesFile) then
        ...
        if fe then
            while true do
                ...
                if ln ~= "" and not ln:match("^#") then
                    -- format: side x y z
                    local side, xs, ys, zs = ln:match("^(%S+)%s+(%-?%d+)%s+(%-?%d+)%s+(%-?%d+)$")
                    if side and xs and ys and zs then
                        local x = tonumber(xs)
                        local y = tonumber(ys)
                        local z = tonumber(zs)
                        local useY = geoSliceY or firstY
...
local function handleStatsMessage(senderID, data)
    ...
    if data.log then
        ...
        -- Detect 'fuel low' log with position (anywhere in the log)
        local x, y, z, facing = line:match("at %((%-?%d+),(%-?%d+),(%-?%d+)%) facing (%d+)")
        if line:lower():find("fuel low") and x and y and z and facing then
            -- Send refuel_job command to fuely (ID 10) in the format fuely expects
            local pos = { x = tonumber(x), y = tonumber(y), z = tonumber(z), facing = tonumber(facing) }
            setStatus("Requesting fuely to refuel turtle at ("..x..","..y..","..z..","..facing..")")
            local logMsg = string.format("[MONITOR] Sent refuel_job to fuely (ID 10) for turtle at (%d,%d,%d) facing %s", pos.x, pos.y, pos.z, tostring(pos.facing))
            appendLog(logMsg)
            rednet.send(fuelyID, { target = pos }, "refuel_job")
        end
        -- Detect handoff complete confirmation from fuely
        if line:find("Handoff complete") then
            setStatus("Fuely reports handoff complete.")
        end
    end
...
-- Dedicated thread for refuel job handling
local function refuelJobListener()
    while true do
...
parallel.waitForAny(refuelJobListener, mainUIThread)
