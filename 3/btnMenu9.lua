...
-- Enhanced Turtle Name Resolver (Label > turtles.cfg > beacon > fallback)
local function getTurtleName(id, info)
    -- 1. Preferred: turtle label (from heartbeat table)
    if info and info.label and info.label ~= "" then
        return info.label
    end

    -- 2. turtles.cfg entry
    if turtleNames[id] then
        return turtleNames[id]
    end

    -- 3. Beacon (DRAWIE / ROOMVA / FIXY / LOGGY)
    if info and info.beacon then
        return info.beacon
    end

    -- 4. Fallback: Turtle<ID>
...
local function saveReceivedMapData(worldmapText, roomsText, blockmapText)
    ...
    if worldmapText then
        local f = fs.open("/maps/monitor_cache/worldmap.txt", "w")
        if f then
            f.write(worldmapText)
            f.close()
        end
    end
    
    -- Save rooms data (fullmap/scans/rooms.txt format)
    if roomsText then
        local f = fs.open("/maps/monitor_cache/rooms.txt", "w")
        if f then
            f.write(roomsText)
            f.close()
        end
...
local function handleTouch(x, y)
            ...
            if x >= listX and x < listX + listWidth and y > listY and y <= listY + listHeight then
                ...
                if roomIdx >= 1 and roomIdx <= #roomvaRooms then
                    ...
                    local success, err = pcall(function()
                        parseRoomMapToMinimap(roomNum)
                    end)
                    if success then
                        logMessage("Viewing Room " .. roomNum, 3)
                    else
                        logMessage("ERROR parsing room " .. roomNum .. ": " .. tostring(err), 0)
                        selectedRoomView = nil
                        -- Send error to computer #3 for logging
                        pcall(function()
                            rednet.send(3, {error="parseRoomMapToMinimap", room=roomNum, msg=tostring(err)}, "ERROR")
                        end)
                    end
                    return
                end
    ...
    if currentPage == MAP_PAGE and y == h-5 then
        ...
        if x >= 1 and x <= 6 then
            ...
            if rid and #roomvaRooms > 0 then
                rednet.send(rid, {roomva_cmd = "goto_room", room_index = roomvaRooms[1]}, "ROOMVA_CMD")
        ...
        if x >= refreshX and x <= refreshX + #refreshLabel - 1 then
            ...
            if rid then
                ...
                rednet.send(rid, { map_request = true }, "ROOMVA_CMD")
                logMessage("Pinging Roomva for map data...", 3)
                
                -- Wait up to 3 seconds for response
                local startTime = os.epoch("local")
                local timeout = 3000  -- 3 seconds in milliseconds
                local receivedResponse = false
                
                while os.epoch("local") - startTime < timeout do
                    local senderId, msg, protocol = rednet.receive(0.1)  -- 0.1s timeout per check
                    if senderId == rid and protocol == "ROOMVA_MAPFILE" and type(msg) == "table" and msg.mapfile then
                        receivedResponse = true
                        logMessage("Received map data from Roomva", 3)
                        -- Parse the map immediately
                        parseWorldmapTextToMinimap(msg.mapfile, msg.blockmap)
                        logMessage("Map refreshed and parsed", 3)
                        break
                    end
                end
                
                if not receivedResponse then
                    logMessage("Roomva timeout - loading local files", 3)
                    loadSavedMapData()
        ...
        if x >= w - 7 and x <= w and y == h-5 then
            ...
            if rid then
                ...
                rednet.send(rid, {roomva_cmd = "explore_on"}, "ROOMVA_CMD")
    ...
    if currentPage == MAP_PAGE and #roomvaRooms > 0 then
        ...
        if x >= listX and x < listX + listWidth and y > listY and y <= listY + listHeight then
            ...
            if clickedIdx <= #roomvaRooms then
                ...
                local rid = getRoomvaId()
                if rid then
                    rednet.send(rid, {roomva_cmd = "goto_room", room_index = selectedRoom}, "ROOMVA_CMD")
    ...
    if currentPage == TLOG_PAGE and y == 6 then
        ...
        -- [RESCAN] button
        if x >= 1 and x <= 8 then
            rednet.send(selectedTurtleId, {roomva_cmd = "explore_on"}, "ROOMVA_CMD")
            ...
            if x >= startX then
                ...
                for i = 1, math.min(#roomvaRooms, 5) do
                    ...
                    if x >= currentX and x < currentX + #btnLabel - 1 then
                        rednet.send(selectedTurtleId, {roomva_cmd = "goto_room", room_index = roomvaRooms[i]}, "ROOMVA_CMD")
                    ...
                    if type(msg) == "table" then
                        ...
                        if proto == "ROOMVA_MAPFILE" and msg.mapfile then
                            ...
                            if shouldUpdate then
                                ...
                                -- Update the map view based on current view mode
                                if selectedRoomView then
                                    -- If viewing a specific room, re-parse that room with new data
                                    local success, err = pcall(function()
                                        parseRoomMapToMinimap(selectedRoomView)
                                    end)
                                    if success then
                                        if shouldLog then
                                            logMessage("Updated Room " .. selectedRoomView .. " view with new data", 3)
                                        end
                                    else
                                        logMessage("ERROR updating room " .. selectedRoomView .. ": " .. tostring(err), 0)
                                        selectedRoomView = nil
                                        -- Send error to computer #3
                                        pcall(function()
                                            rednet.send(3, {error="parseRoomMapToMinimap_update", room=selectedRoomView, msg=tostring(err)}, "ERROR")
                                        end)
                                    end
                                else
                                    -- Otherwise parse the full map
                                    parseWorldmapTextToMinimap(msg.mapfile, msg.blockmap)
                                    if shouldLog then
                                        logMessage("Updated Roomva minimap from worldmap.", id)
                                    end
                                end
                            end
