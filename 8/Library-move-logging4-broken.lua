...
local function heartbeatThread()
    while true do
        local startTime = os.epoch("local") / 1000
        ...
        if x and y and z then
            -- Get turtle name/label
            local name = os.getComputerLabel() or ("Turtle_" .. os.getComputerID())
            ...
            rednet.send(compID, packet, "HB")
        end
        
        -- Sleep for remainder of interval
        local elapsed = (os.epoch("local") / 1000) - startTime
        local sleepTime = math.max(0.05, HEARTBEAT_INTERVAL - elapsed)
        sleep(sleepTime)
    end
end
...
local function mainLoop()
    while true do
        if refreshFlag then local where = routeRecover() end
        where = routeRecover()
