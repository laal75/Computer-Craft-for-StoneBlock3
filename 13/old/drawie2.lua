...
-- Traverse full path forward (L1 to L6)
local function traverseLighthousesForward()
    local current = "L1"
    goToLighthouse(current)
    for _=1,5 do
        current = stepLighthouseForward(current)
    end
end

-- Traverse full path backward (L6 to L1)
local function traverseLighthousesBackward()
    local current = "L6"
    goToLighthouse(current)
    for _=1,5 do
        current = stepLighthouseBackward(current)
    end
end
...
local function mainLoop()
    ...
    -- Go from L1 to L5 (deposit area) at start
    GotoL6("L1")
    ...
    while true do
        ...
        -- After sorting, return to L1 (wood source)
        GotoL1("L5")
        sleep(SORT_INTERVAL)
        -- Go back to L5 for next pass
        GotoL6("L1")
    end
end
