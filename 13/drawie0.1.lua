-- ======================================================
-- Drawie v2.1 — Drawer-to-Drawer Sorting Turtle
-- - Starts parked facing away from empty drawer wall
-- - Uses A.lua GPS (A.moveTo) to travel between two walls
-- - Pulls items from FULL drawers wall, puts them into EMPTY wall
-- - Automatically assigns each item type to a permanent drawer slot
-- - Sends heartbeats (HB) like Fixy so it shows on Fleet/Monitor
-- - Logs moves back to monitor 3 on "stats" channel
-- - Responds to MAINT logic similar to Fixy:
--      maint_cmd = "visit"       → one-shot sort pass
--      maint_cmd = "halt"        → pause auto-sorting
--      maint_cmd = "resume"      → resume auto-sorting
--      maint_cmd = "return_home" → go back to park
--      maint_cmd = "poke"/"restart"/"pulse" → redstone pulses
-- ======================================================
...
-- Adjust these if your A.lua uses different numbers.
-- Common setup is:
...
-- turtle is looking directly at the drawers.
-- (Same XYZ as home, but facing East into the wall.)
local destBase = {
    x   = 20,
    y   = 0,
    z   = -17,
...
-- FULL drawers wall (ores / gems): position in front of BOTTOM-LEFT drawer,
-- turtle looking directly at drawers.
local srcBase = {
    x   = 6,
    y   = 1,
    z   = -26,
...
-- Drawer grid sizes.
-- Adjust if your walls are not 6x8.
...
-- Using normal Minecraft coords: X+ = East, Z+ = South
local dirDx = {
    [DIR_N] =  0,
    [DIR_W] = -1,
    [DIR_S] =  0,
...
local function resolveName(id, info)
...
local function mainLoop()
    ...
    print("=== Drawie v2.1 — Drawer Sorter ===")
